...
function isServerActive(msg) {
  exec("systemctl status valheimserver.service | egrep Active | sed 's/^[[:space:]]*//'", (err, stdout) => {
    if (err) {
      console.log(err);
    } else {
      const output = stdout;
      const stat = output.split(' ')[1];
...
function showOnlinePlayers() {
  ...
  const files = fs.readdirSync('playerlist/online/');
...
function showOfflinePlayers() {
  ...
  const files = fs.readdirSync('playerlist/offline/');
